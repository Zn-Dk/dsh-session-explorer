...
### 已知问题
- 会话总览的搜索结果点击命中 badge 后，不会自动滚动定位到该消息并高亮；当前仅选中对应会话，需要手动在详情中查找。

## [0.4.1] - 2026-08-27

### 新增
- 时间线详情顶部增加「上一条 / 下一条」会话切换器（跟随当前筛选/排序后的可见列表，含位置指示）。
- 会话卡片首次加载时逐个淡入上浮（stagger 进场动画，克制 0.22s / 14ms 递增，支持 prefers-reduced-motion）。

### 变更
- 消息摘要展开/收起改为即时切换（移除高度伸缩动画），避免折叠态多余空白与展开卡顿。
- 消息摘要加载中由纯文本改为骨架屏（脉冲占位）。

### 修复
- 修复会话详情头部「子代理」badge 被 flex 拉伸变形（detail-head 加 align-items: center）。
- 修复时间线初始化时因变量声明顺序导致的渲染崩溃（TDZ ReferenceError，表现为点击时间线空白）。
